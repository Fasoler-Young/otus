...
public class MessageController {
    ...
    private static final String ADMIN_ROOM_ID = "1408";
    ...
    //    @MessageMapping("/message." + ADMIN_ROOM_ID)
    //    public void getMessageForAdmin(Message message) {
    //        logger.info("get message:{}, roomId:{}", message, ADMIN_ROOM_ID);
    //        saveMessage(ADMIN_ROOM_ID, message).subscribe(msgId -> logger.info("message send id:{}", msgId));
    //
    //        template.convertAndSend(
    //                String.format("%s%s", TOPIC_TEMPLATE, ADMIN_ROOM_ID), new
    // Message(HtmlUtils.htmlEscape(message.messageStr())));
    //    }

    @MessageMapping("/message.{roomId}")
    public void getMessage(@DestinationVariable("roomId") String roomId, Message message) {
        logger.info("get message:{}, roomId:{}", message, roomId);
        if (ADMIN_ROOM_ID.equals(roomId)) {
            return;
        }
        ...
        template.convertAndSend(
                String.format("%s%s", TOPIC_TEMPLATE, ADMIN_ROOM_ID),
